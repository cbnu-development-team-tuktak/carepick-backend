...
class DiseaseEntityController(
    ...
    // 예: http://localhost:8080/api/diseases/processed/count
    ...
    fun getDiseaseById(@PathVariable id: Long): ResponseEntity<DiseaseDetailsResponse> {
        // ID를 기준으로 해당 질병 엔티티를 조회
        val disease = diseaseService.findDiseaseById(id)

        return if (disease != null) {
            // 조회된 질병 데이터를 DTO로 변환
            val dto = DiseaseDetailsResponse.from(disease)
            // 변환된 DTO를 응답으로 반환 
            ResponseEntity.ok(dto)
        } else {
            // 해당 ID의 질병 정보가 없을 경우 404 Not Found 응답 반환

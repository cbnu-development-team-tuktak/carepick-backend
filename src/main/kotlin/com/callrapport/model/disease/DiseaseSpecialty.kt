...
@Entity
@Table(name = "disease_specialties")
data class DiseaseSpecialty(
    @Id // 기본 키 설정
    @GeneratedValue(strategy = GenerationType.IDENTITY) // 자동 증가
    val id: Long? = null, // 질병-진료과 관계 ID

    // 질병(Disease)과 다대일(N:1) 관계
    @ManyToOne // 여러 개의 DiseaseSpecialty가 하나의 Disease에 연결될 수 있음
    @JoinColumn(
    ...
    @ManyToOne // 여러 개의 DiseaseSpecialty가 하나의 Specialty에 연결될 수 있음
    @JoinColumn(
        name = "specialty_id", // 매핑할 외래 키(FK) 이름
        nullable = false, // 필수 입력 값 (NULL 허용 안 함)
        referencedColumnName = "id" // Specialty 엔티티의 "id" 컬럼을 참조
    )
    val specialty: Specialty // 해당 질병이 속하는 진료과
)
